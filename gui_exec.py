...
def open_Main_windows():
    ...
    def run_command_synth():
        command = 'RplGenCore.exe --Modules speech_synthesizer --LogFile {lg} --MediaObjDefine {md} --CharacterTable {ct} --OutputPath {of} --AccessKey {AK} --AccessKeySecret {AS} --Appkey {AP} --Azurekey {AZ} --ServRegion {SR}'
    ...
    def run_command_synth_preview(init_type='Aliyun'):
        command = 'RplGenCore.exe --Modules speech_synthesizer --PreviewOnly --Init {IN} --AccessKey {AK} --AccessKeySecret {AS} --Appkey {AP} --Azurekey {AZ} --ServRegion {SR}'
...
if __name__=='__main__':
    os.system("echo TRPG-Replay-Generator version 1.0.4 (6 Aug 2022)")
